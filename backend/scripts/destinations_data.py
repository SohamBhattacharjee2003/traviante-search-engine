"""Traviante destination catalog.

This is the seed data used in two places:
  1. Mock mode — loaded into the in-memory metadata store so the stack runs locally.
  2. The indexing pipeline (``scripts/index_destinations.py``) — embeds + upserts them.

``DESTINATIONS`` resolves to the **real catalog** in ``scripts/catalog.json`` when it
exists (generated by ``scripts/build_catalog_from_kaggle.py``); otherwise it falls back
to the curated ``SAMPLE_DESTINATIONS`` below so the demo always renders out of the box.

Sample images point at Unsplash (royalty-free). In production these would be Cloudinary
URLs.
"""
...
SAMPLE_DESTINATIONS: list[Destination] = [
    Destination(
        id="bali-ubud",
        name="Ubud, Bali",
        country="Indonesia",
...
def _load_catalog() -> list[Destination]:
    """Prefer the generated real catalog; fall back to the curated samples."""
    if _CATALOG_PATH.exists():
        try:
            raw = json.loads(_CATALOG_PATH.read_text())
            catalog = [Destination(**row) for row in raw]
            if catalog:
                logger.info(
                    "Loaded %d destinations from %s", len(catalog), _CATALOG_PATH.name
                )
                return catalog
        except Exception as exc:  # malformed file shouldn't break startup
            logger.warning("Failed to load %s (%s); using samples.", _CATALOG_PATH.name, exc)
    return SAMPLE_DESTINATIONS


DESTINATIONS: list[Destination] = _load_catalog()
