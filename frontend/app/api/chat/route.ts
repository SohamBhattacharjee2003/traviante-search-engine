// Server-side proxy for the conversational concierge (text turns).
// Forwards the JSON chat payload to the FastAPI backend, keeping BACKEND_URL secret.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = new URL(`${BACKEND_URL}/v1/chat`);
